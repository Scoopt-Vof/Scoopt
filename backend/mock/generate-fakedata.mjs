...
// ---------------------------------------------------------------------------
// ALSO emit per-retailer seed files in RawOffer shape, so the SAME catalogue
// can be ingested into Postgres. That is what makes the trial run meaningful:
// the identical 49 products reach the front end either from a TypeScript file
// or from a real database, and the site cannot tell which.
// ---------------------------------------------------------------------------
mkdirSync(new URL('../data/seed/', import.meta.url), { recursive: true });
const SUB_TO_CAT = { hardlopen: 'hardlopen', fietsen: 'fietsen', fitness: 'fitness' };
let seedTotal = 0;
for (const store of STORES) {
  const rows = products
    .filter((p) => STOCKS[store].includes(p.brand))
    .map((p) => ({
      retailerSku: `${store}-${p.id}`,
      ean: p.ean,
      brand: p.brand,
      title: p.name,
      // category is the CONTRACT category (home|sport|tech) — NOT the subcategory.
      // Setting this to "hardlopen" made /api/category/sport return zero
      // subcategories, so the whole browse page came back empty.
      category: 'sport',
      priceCents: Math.round(priceFor(p.id, p.rrp, store) * 100),
      shippingCents: 0,
      currency: 'EUR',
      inStock: hash(p.id + store + 'stock') % 11 !== 0,
      productUrl: `https://example.invalid/out?store=${store}&pid=${p.id}`,
      imageUrl: null,
      description: null,
      // contract extras
      unit: p.unit,
  ...
  writeFileSync(
    new URL(`../data/seed/${store.replace(/\./g, '-')}.json`, import.meta.url),
    JSON.stringify({ store, generatedAt: null, source: 'FIXTURE — invented prices', products: rows }, null, 2)
  );
}
console.log(`wrote ${seedTotal} seed offers across ${STORES.length} retailer files`);
