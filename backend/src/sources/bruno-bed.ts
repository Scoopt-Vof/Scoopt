...
/**
 * Bruno Bed NL (brunobed.nl) — Awin affiliate PRODUCT FEED adapter.
 *
 * Beds, mattresses and home/bedroom furniture. Structurally identical to the
 * GSM Net adapter (see gsmnet.ts and README-AWIN.md): the same Awin
 * Create-a-Feed, the same streaming CSV parse, only the retailer identity and
 * env var differ. Because everything downstream of RetailerSource is
 * retailer-agnostic (./types.ts), this one file is all the backend needs for
 * Bruno Bed.
 *
 ...
 * Bruno Bed NL, language Dutch, CSV / comma / gzip, columns including
 ...
 * description. Put the feed URL in .env as BRUNOBED_FEED_URL.
 * Run: npm run ingest -- bruno-bed
 */

const DEFAULT_CATEGORY = (process.env.BRUNOBED_CATEGORY ?? 'home').toLowerCase();
...
export class BrunoBedSource implements RetailerSource {
  readonly slug = 'bruno-bed';
  readonly name = 'Bruno Bed';
  readonly homepageUrl = 'https://www.brunobed.nl';
  ...
  constructor(
    ...
    private readonly delimiter: string = process.env.BRUNOBED_FEED_DELIMITER ?? ','
  ) {}

  async fetch(): Promise<FetchResult> {
    const tmp = join(tmpdir(), `bruno-bed-feed-${process.pid}-${Date.now()}.bin`);

    const res = await fetch(this.feedUrl, { headers: { 'user-agent': UA, accept: '*/*' } });
    if (!res.ok || !res.body) {
      throw new Error(`HTTP ${res.status} downloading Bruno Bed feed from ${hostOf(this.feedUrl)}`);
    }
      ...
      const parser = makeRowParser(this.delimiter, (row) => {
        if (!index) {
          ...
          if (col(index, COLUMNS.url) < 0) {
            throw new Error(
              `Bruno Bed feed has no deep-link column. Add "aw_deep_link" in Create-a-Feed ` +
...
function requireFeedUrl(): string {
  const url = process.env.BRUNOBED_FEED_URL;
  if (!url) {
    throw new Error(
      'BRUNOBED_FEED_URL is not set.\n' +
      '  → Awin → Toolbox → Create-a-Feed → Bruno Bed NL → copy the feed URL into .env'
    );
  }
  return url;
}
