...
/**
 * JD Sports NL (jdsports.nl) — Awin affiliate PRODUCT FEED adapter.
 *
 * Sportswear and trainers. Structurally identical to the GSM Net adapter
 * (see gsmnet.ts and README-AWIN.md): the same Awin Create-a-Feed, the same
 * streaming CSV parse, only the retailer identity and env var differ. Because
 * everything downstream of RetailerSource is retailer-agnostic (./types.ts),
 * this one file is all the backend needs for JD Sports.
 *
 ...
 * JD Sports NL, language Dutch, CSV / comma / gzip, columns including
 ...
 * description. Put the feed URL in .env as JDSPORTS_FEED_URL.
 * Run: npm run ingest -- jd-sports
 */

const DEFAULT_CATEGORY = (process.env.JDSPORTS_CATEGORY ?? 'sport').toLowerCase();
...
export class JdSportsSource implements RetailerSource {
  readonly slug = 'jd-sports';
  readonly name = 'JD Sports';
  readonly homepageUrl = 'https://www.jdsports.nl';
  ...
  constructor(
    ...
    private readonly delimiter: string = process.env.JDSPORTS_FEED_DELIMITER ?? ','
  ) {}

  async fetch(): Promise<FetchResult> {
    const tmp = join(tmpdir(), `jd-sports-feed-${process.pid}-${Date.now()}.bin`);

    const res = await fetch(this.feedUrl, { headers: { 'user-agent': UA, accept: '*/*' } });
    if (!res.ok || !res.body) {
      throw new Error(`HTTP ${res.status} downloading JD Sports feed from ${hostOf(this.feedUrl)}`);
    }
      ...
      const parser = makeRowParser(this.delimiter, (row) => {
        if (!index) {
          ...
          if (col(index, COLUMNS.url) < 0) {
            throw new Error(
              `JD Sports feed has no deep-link column. Add "aw_deep_link" in Create-a-Feed ` +
...
function requireFeedUrl(): string {
  const url = process.env.JDSPORTS_FEED_URL;
  if (!url) {
    throw new Error(
      'JDSPORTS_FEED_URL is not set.\n' +
      '  → Awin → Toolbox → Create-a-Feed → JD Sports NL → copy the feed URL into .env'
    );
  }
  return url;
}
