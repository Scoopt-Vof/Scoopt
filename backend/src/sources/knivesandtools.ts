...
/**
 * Knivesandtools NL (knivesandtools.nl) — Awin affiliate PRODUCT FEED adapter.
 *
 * Knives, tools, outdoor and kitchen gear. Structurally identical to the GSM
 * Net adapter (see gsmnet.ts and README-AWIN.md): the same Awin Create-a-Feed,
 * the same streaming CSV parse, only the retailer identity and env var differ.
 * Because everything downstream of RetailerSource is retailer-agnostic
 * (./types.ts), this one file is all the backend needs for Knivesandtools.
 *
 * MEMORY: streams the (potentially large) feed to a temp file, then
 * decompresses and parses it row by row, holding only one row plus the growing
 * offers list — peak memory is bounded and independent of feed size. See the
 * note in gsmnet.ts for the OOM history this avoids.
 *
 * SETUP (see README-AWIN.md): Awin > Toolbox > Create-a-Feed, advertiser
 * Knivesandtools NL, language Dutch, CSV / comma / gzip, columns including
 * aw_deep_link, product_name, aw_product_id, ean, product_GTIN, brand_name,
 * search_price, currency, in_stock, aw_image_url, merchant_category,
 * description. Put the feed URL in .env as KNIVESANDTOOLS_FEED_URL.
 * Run: npm run ingest -- knivesandtools
 */

const DEFAULT_CATEGORY = (process.env.KNIVESANDTOOLS_CATEGORY ?? 'home').toLowerCase();
...
export class KnivesandtoolsSource implements RetailerSource {
  readonly slug = 'knivesandtools';
  readonly name = 'Knivesandtools';
  readonly homepageUrl = 'https://www.knivesandtools.nl';
  ...
  constructor(
    ...
    private readonly delimiter: string = process.env.KNIVESANDTOOLS_FEED_DELIMITER ?? ','
  ) {}

  async fetch(): Promise<FetchResult> {
    const tmp = join(tmpdir(), `knivesandtools-feed-${process.pid}-${Date.now()}.bin`);

    const res = await fetch(this.feedUrl, { headers: { 'user-agent': UA, accept: '*/*' } });
    if (!res.ok || !res.body) {
      throw new Error(`HTTP ${res.status} downloading Knivesandtools feed from ${hostOf(this.feedUrl)}`);
    }
      ...
      const parser = makeRowParser(this.delimiter, (row) => {
        if (!index) {
          ...
          if (col(index, COLUMNS.url) < 0) {
            throw new Error(
              `Knivesandtools feed has no deep-link column. Add "aw_deep_link" in Create-a-Feed ` +
...
function requireFeedUrl(): string {
  const url = process.env.KNIVESANDTOOLS_FEED_URL;
  if (!url) {
    throw new Error(
      'KNIVESANDTOOLS_FEED_URL is not set.\n' +
      '  → Awin → Toolbox → Create-a-Feed → Knivesandtools NL → copy the feed URL into .env'
    );
  }
  return url;
}
